<div wire:key="main-product-wrapper" class="bg-light min-vh-100 pb-5">
    ...
    <div class="container-xl pt-4">
        ...
        <div class="row g-4">
                    ...
                    <div class="d-md-none px-2 mb-4">
                        <div class="mobile-dropdown-group mb-2">
                            <div class="custom-hover-dropdown">
                                ...
                                <div class="dropdown-content shadow-lg rounded-3 border">
                                    <button wire:click="$set('selectedCategoryId', 'All')" class="dropdown-item py-2 border-bottom {{ $selectedCategoryId == 'All' ? 'bg-primary text-white' : '' }}">All Brands</button>
                                    @foreach($categories as $category)
                                        <button wire:click="$set('selectedCategoryId', '{{ $category->id }}')" class="dropdown-item py-2 border-bottom {{ $selectedCategoryId == $category->id ? 'bg-primary text-white' : '' }}">{{ $category->name }}</button>
                                    @endforeach
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="custom-hover-dropdown">
                                ...
                                <div class="dropdown-content shadow-lg rounded-3 border" style="max-height: 250px; overflow-y: auto;">
                                    @forelse ($devices as $device)
                                        <button wire:click="selectDevice('{{ $device->id }}')" class="dropdown-item py-2 border-bottom d-flex justify-content-between">
                        ...
                        <div class="bg-white p-3 rounded-3 border shadow-sm">
                            <div class="d-flex justify-content-between mb-2">
                                ...
                                <span class="small fw-bold">£{{ number_format($price ?? 12000) }}</span>
                            </div>
                    ...
                    <div class="d-none d-md-block">
                        <div class="filter-group mb-4">
                            ...
                            <div class="brand-stack">
                                <div class="brand-box {{ $selectedCategoryId == 'All' ? 'active' : '' }}" wire:click="$set('selectedCategoryId', 'All')">
                                    All Brands
                                </div>
                                @foreach ($categories as $category)
                                    <div class="brand-box {{ $selectedCategoryId == $category->id ? 'active' : '' }}" wire:click="$set('selectedCategoryId', '{{ $category->id }}')">
                                        {{ $category->name }}
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="filter-group mb-4">
                            ...
                            <div class="card border-0 shadow-sm rounded-3 overflow-hidden">
                                <div class="custom-scrollbar" style="max-height: 280px; overflow-y: auto;">
                                    @forelse ($devices as $device)
                                        <div class="model-item d-flex justify-content-between align-items-center border-bottom p-3" wire:click="selectDevice('{{ $device->id }}')" style="cursor: pointer;">
                                            <span class="fw-semibold text-dark small">{{ $device->name }}</span>
                        ...
                        <div class="price-container p-3 bg-white rounded-3 border shadow-sm">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                ...
                                <span class="fw-bold text-dark">£{{ number_format($price ?? 12000) }}</span>
                            </div>
                            <input type="range" class="form-range custom-range" min="0" max="12000" step="100" wire:model="price">
                        </div>
                    </div>
            ...
            <div class="col-md-8 col-lg-9">
               ...
               @forelse(collect($models)->sortByDesc(function ($item) {
    ...
    return ($number * 10) + $priority;
                    ...
                    <div class="col" wire:key="prod-{{ $model['id'] }}">
                        {{-- ✅ SEO SLUG ROUTE — category_slug / device_slug / model_slug --}}
                        <a href="{{ route('guest-buy-product-specs', [
                            ...
                            <div class="card h-100 border-0 rounded-4 product-card shadow-sm p-3">
                                <div class="image-box mb-3 text-center">
                                    <img src="{{ $model['file'] ?? asset('assets/no-img.png') }}" class="img-fluid" style="height: 130px; object-fit: contain;">
                                </div>
                                <div class="card-content text-center">
                                    <h6 class="product-name text-dark fw-bold mb-2">{{ $model['name'] }}</h6>
