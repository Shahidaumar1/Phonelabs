<div>
    ...
    <div class="container main-wrapper">
            ...
            <div class="row g-3 d-flex align-items-stretch">
                ...
                <div class="col-12 col-lg-4">
                    <div class="product-image-box shadow-sm">
                        <img src="{{ $product_spec_image ?? $model->file }}" class="img-fluid" style="max-height: 250px; width: auto; object-fit: contain;">
                    </div>
                </div>

                {{-- RIGHT: SELECTION BOX (NOW WHITE) --}}
                    ...
                    <div class="selection-panel-box shadow-sm">
                                ...
                                <div class="d-flex gap-2 flex-wrap">
                                    @if (!empty($available_memory_sizes))
                                        @foreach ($available_memory_sizes as $memory_size)
                                            <button type="button" class="btn-option {{ $memory_size == $selectedMemorySize ? 'active-spec' : '' }}" wire:click="$set('selectedMemorySize', '{{ $memory_size }}')">
                                                {{ $memory_size }}
                                            </button>
                                        @endforeach
                                    @endif
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <div class="d-flex gap-2 flex-wrap">
                                    @foreach ($available_grades as $grade)
                                        <button type="button" class="btn-option {{ $grade == $selectedGrade ? 'active-spec' : '' }}" wire:click="$set('selectedGrade', '{{ $grade }}')">
                                            {{ $grade }}
                                        </button>
                                    @endforeach
                                </div>
                                ...
                                <div class="d-flex gap-2 flex-wrap">
                                    @if (!empty($available_colors))
                                        @foreach ($available_colors as $color)
                                            <button type="button" class="btn-option {{ $color == $selectedColor ? 'active-spec' : '' }}" wire:click="$set('selectedColor', '{{ $color }}')">
                                                {{ $color }}
                                            </button>
                                        @endforeach
                                    @endif
                        ...
                        <div class="row g-3 row-separator align-items-end">
                            ...
                            <div class="col-6 col-md-4">
                                ...
                                <input type="text" class="form-control-custom" style="font-weight:bold; height: 40px;" value="£{{ $price ?? 0 }}.00" readonly>
                            </div>
                        ...
                        <div class="row g-2 mt-3" x-data="{ openTab: 0 }">
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <div class="accordion-content-box" x-show="openTab === 3" x-cloak x-transition>
                                    {!! $warranty ?? 'No warranty info available.' !!}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-3">
                            <klarna-placement 
                                ...
                                data-locale="en-GB" 
                                data-purchase-amount="{{ ($price ?? 0) * 100 }}">
                            </klarna-placement>
                        </div>
                    </div>
                </div>
        ...
        {{-- OTHER SECTIONS --}}
        <section id="form-toggle-section" class="form-step" style="display:none;"><livewire:guest.components.form-toggle :data="$data" :key="uniqid()" /></section>
        <section id="patient-detail-section" class="form-step" style="display:none;"><livewire:guest.components.patient-detail-form :key="uniqid()" /></section>
        <section id="booking-section" class="form-step" style="display:none;"><livewire:guest.buy.book-repair :data="$data" :key="uniqid()" /></section>
    </div>
