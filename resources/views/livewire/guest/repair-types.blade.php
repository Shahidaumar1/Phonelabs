...
<div class="cust-container">
    ...
    <header class="repair-header">
        ...
        <p style="color: var(--text-muted); font-weight: 500;">Select the repair you need for your {{ $modal->name ?? 'device' }}</p>
    </header>

    <div class="repair-grid">
        @if ($device && $device->repair_types)
            @foreach ($device->repair_types as $repair_type)
                @php
    ...
    'repair' => $repairSeo,
                ...
                @if ($priceAmount > 0)
                <a href="{{ $targetUrl }}" class="repair-card">
                    <div class="card-media">
                        <img src="{{ asset($repair_type->image) }}" alt="{{ $repair_type->name }}" loading="lazy">
                    </div>

                    <div class="card-content">
                        <h3 class="repair-title">{{ $repair_type->name }}</h3>

                        <div class="price-box">
                            {{-- Price: sirf normal pe dikhao --}}
                            @if ($isNormal)
                                <span class="repair-price">£{{ number_format($priceAmount, 2) }}</span>

                                {{-- Klarna: normal price >= 30 pe --}}
                                @if ($showKlarna)
                                    <br>
