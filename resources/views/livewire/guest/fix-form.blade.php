                ...
                <div class="row justify-content-center">
                    <div class="col-lg-6 col-md-8 col-sm-10">
                        ...
                        <div class="date-selection-card">
                                ...
                                <div class="calendar-trigger-left">
                                    ...
                                    <div class="calendar-text">
                                        ...
                                        <div class="selected-date">
                                            {{ $fixed['repair_date'] ?? 'Choose a date' }}
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="calendar-modal-body">
                                        <div class="cute-calendar">
                                            <div class="calendar-month-nav">
                                                ...
                                                <span class="current-month">{{ $this->currentMonth->format('F Y') }}</span>
                                            ...
                                            <div class="calendar-days">
                                                @if($this->calendarDays)
                                                    @foreach ($this->calendarDays as $week)
                                                        @foreach ($week as $day)
                                                            <div class="calendar-day {{ $day['class'] }} {{ $day['disabled'] ? 'disabled' : '' }}"
                                                                @if(!$day['disabled'])
                            ...
                            <span class="text-danger" style="font-size: 11px; margin-top: 8px; display: block;">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="time-selection-card">
                            ...
                            <div class="time-grid">
                                ...
                                <div class="time-slot {{ $fixed['repair_time'] === $timeslots['time'] ? 'active' : '' }}"
                                    wire:click="$set('fixed.repair_time', '{{ $timeslots['time'] }}')">
                                    <i class="{{ $timeslots['icon'] }} time-icon"></i>
                                    <span class="time-value">{{ $timeslots['time'] }}</span>
                                    <span class="time-period-badge">
                                        @if(strpos($timeslots['time'], '09') !== false || strpos($timeslots['time'], '10') !== false)
                                            Morning
                                        @elseif(strpos($timeslots['time'], '12') !== false || strpos($timeslots['time'], '13') !== false)
                                            Lunch
                                        @elseif(strpos($timeslots['time'], '14') !== false || strpos($timeslots['time'], '15') !== false)
                                            Afternoon
                                        @elseif(strpos($timeslots['time'], '16') !== false || strpos($timeslots['time'], '17') !== false)
                                            Evening
                                        @else
                                            Available
                                        @endif
                        ...
                        <div class="selected-summary">
                            <p>
                                ...
                                <strong>Date:</strong> {{ $fixed['repair_date'] ?? 'Not selected' }} &nbsp;|&nbsp;
                                <strong>Time:</strong> {{ $fixed['repair_time'] ?? 'Not selected' }}
                            </p>
                        </div>
