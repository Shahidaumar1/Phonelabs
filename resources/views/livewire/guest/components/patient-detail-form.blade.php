        ...
        <div class="custom-container">
            ...
            <div class="form-card">
                <form wire:submit.prevent="submitForm">
                    ...
                    @error('patient.name') <div class="text-danger">{{ $message }}</div> @enderror

                    <div class="input-wrapper">
                        <i class="fa-regular fa-envelope"></i>
                        <input type="email" placeholder="Email Address" wire:model.lazy="patient.email">
                    </div>
                    @error('patient.email') <div class="text-danger">{{ $message }}</div> @enderror

                    <div class="input-wrapper">
                        <i class="fa-solid fa-phone"></i>
                        <input type="tel" placeholder="Phone Number" wire:model.lazy="patient.phone">
                    </div>
                    @error('patient.phone') <div class="text-danger">{{ $message }}</div> @enderror

                    <div class="input-wrapper textarea-wrap">
                        <i class="fa-regular fa-comment-dots"></i>
                        <textarea wire:model.lazy="patient.RepairNote" placeholder="Notes..."></textarea>
                    </div>
                    @error('patient.RepairNote') <div class="text-danger">{{ $message }}</div> @enderror

                    <div class="btn-container">
                        <button type="submit" class="payment-method" 
                            @if(empty($patient['name']) || empty($patient['email']) || empty($patient['phone'])) disabled @endif>
