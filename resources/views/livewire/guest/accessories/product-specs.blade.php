            ...
            <main class="app-card" id="main-interface">
                <div class="app-content-body">
                    ...
                    <section id="product-info-section" class="form-step-content">
                        <div class="product-flex-layout">
                            <div class="img-container">
                                @php $images = json_decode($product_spec_image); @endphp
                                @if (empty($images))
                                    <img src="{{ $product_spec_image ?? $model->file }}" class="product-img" alt="{{ $model->name }}">
                                @else
                                    <div id="imageSlider" class="carousel slide w-100" data-bs-ride="carousel">
                                        <div class="carousel-inner">
                                            @foreach ($images as $index => $image)
                                                <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                                                    <img src="{{ $image }}" class="product-img d-block mx-auto" alt="Product View">
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
                            ...
                            <div class="product-info-panel">
                                <h2 class="fw-bold mb-1" style="color: var(--slate-900);">{{ $model->name }}</h2>
                                ...
                                @if (!empty($available_memory_sizes))
                                    <div>
                                        ...
                                        <div class="spec-pill-grid">
                                            @foreach ($available_memory_sizes as $size)
                                                <div class="spec-pill {{ $size == $selectedMemorySize ? 'active' : '' }}"
                                                     wire:click="$set('selectedMemorySize', '{{ $size }}')">
                                                    {{ $size }}
                                                </div>
                                            @endforeach
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="spec-pill-grid">
                                            @foreach ($available_colors as $color)
                                                <div class="spec-pill {{ $color == $selectedColor ? 'active' : '' }}"
                                ...
                                <div class="row align-items-end g-3">
                                    <div class="col-sm-6">
                                        ...
                                        <div class="input-group" style="max-width: 140px;">
                                            ...
                                            <input type="text" class="form-control text-center border-2 bg-white fw-bold" value="{{ $quantity }}" readonly>
                                    ...
                                    <div class="col-sm-6">
                                        <div class="value-box">
                                            ...
                                            <span class="h3 mb-0 fw-bold">£{{ number_format($price ?? 0, 2) }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <section id="form-toggle-section" class="form-step-content" style="display:none;">
                        <livewire:guest.components.form-toggle :data="$data" :key="'step1-'.uniqid()" />
                    </section>

                    {{-- STEP 2 --}}
                    <section id="patient-detail-section" class="form-step-content" style="display:none;">
                        <livewire:guest.components.patient-detail-form :key="'step2-'.uniqid()" />
                    </section>

                    {{-- STEP 3 --}}
                    <section id="booking-section" class="form-step-content" style="display:none;">
                        <livewire:guest.accessories.book-repair :data="$data" :key="'step3-'.uniqid()" />
                    </section>
                </div>
