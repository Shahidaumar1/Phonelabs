...
class Create extends Component
{
    ...
    public function save()
    {
        ...
        $this->validate([
            'name' => [
                Rule::unique('device_types', 'name')->where(function ($query) {
                    return $query->where('service', ServiceType::UNLOCKING)
        ...
        $device->service = ServiceType::UNLOCKING;
        $device->name = $this->name;
        $device->category_id = $this->category->id;
        $device->save();
        $this->emit('closeM', 'add-unlock-device');
    ...
    {
        return view('livewire.admin.unlocking.device.create');
    }
}
