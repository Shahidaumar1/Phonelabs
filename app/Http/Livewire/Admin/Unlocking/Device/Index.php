...
namespace App\Http\Livewire\Admin\Unlocking\Device;
...
class Index extends Component
{
    ...
    public function selectDevice(DeviceType $deivce)
    {
        ...
        $this->emit('showM', 'edit-unlock-device');
    }
    ...
    public function fetchData($args)
    {
        $this->target = $args;
        if ($args == 'Junk') {
            $this->devices = $this->selectedCategory ? DeviceType::onlyTrashed()->where('service', ServiceType::UNLOCKING)->get() : [];
        } else {
    ...
    {
        return view('livewire.admin.unlocking.device.index')->layout('layouts.admin');
    }
}
