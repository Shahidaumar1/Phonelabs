...
class Index extends Component
{
    ...
    public function fetchData($args)
    {
        $this->target = $args;
        if ($args == 'Junk') {
            $this->categories = Category::onlyTrashed()->where('service', ServiceType::UNLOCKING)->get();
        } else {
            $this->categories = Category::where('service', ServiceType::UNLOCKING)->where('status', $args)->get();
        }
    }
    ...
    public function selectCat(Category $category)
    {
        ...
        $this->emit('showM', 'edit-unlock-cat');
    }
    ...
    {
        return view('livewire.admin.unlocking.category.index')->layout('layouts.admin');
    }
}
