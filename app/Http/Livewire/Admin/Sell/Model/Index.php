...
class Index extends Component
{
    ...
    public function selectModel(Modal $modal)
    {
        ...
        $this->emit('showM', 'edit-sell-model');
    }
    ...
    public function fetchData($args)
    {
        $this->target = $args;
        if ($args == 'Junk') {
            $this->models = $this->selectedDevice ? Modal::onlyTrashed()->where('service', ServiceType::SELL)->get() : [];
        } else {
            $this->models = $this->selectedDevice ? Modal::where('device_type_id', $this->selectedDevice->id)->where('status', $args)->get() : [];

        }
        if ($args == 'Top Rated') {
            $this->models = $this->selectedDevice ? Modal::where('service', ServiceType::SELL)->where('top_rated', true)->get() : [];
        }
        if ($args == 'New Arrival') {
            $this->models = $this->selectedDevice ? Modal::where('service', ServiceType::SELL)->where('new_arrival', true)->get() : [];
        }
    ...
    {
        return view('livewire.admin.sell.model.index')->layout('layouts.admin');
    }
}
