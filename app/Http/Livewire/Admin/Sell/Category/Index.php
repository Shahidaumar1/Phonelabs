...
class Index extends Component
{
    ...
    public function fetchData($args)
    {
        $this->target = $args;
        if ($args == 'Junk') {
            $this->categories = Category::onlyTrashed()->where('service', ServiceType::SELL)->get();
        } else {
            $this->categories = Category::where('service', ServiceType::SELL)->where('status', $args)->get();
        }
    ...
    public function addConditionText($categoryId)
    {
        $this->emit('categoryId', $categoryId);
        $this->emit('showM', 'add-condition-text');
    }

    public function render()
    {
        return view('livewire.admin.sell.category.index')->layout('layouts.admin');
    }
}
