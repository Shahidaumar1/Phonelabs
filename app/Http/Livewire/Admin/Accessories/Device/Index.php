...
namespace App\Http\Livewire\Admin\Accessories\Device;
...
class Index extends Component
{
    ...
    public function updateOrderNumber($orders)
    {
        foreach ($orders as $order) {
            DeviceType::where('id', (int)$order['value'])->update(['order_number' => $order['order']]);
        }
    ...
    public function selectDevice(DeviceType $deivce)
    {
        ...
        $this->emit('showM', 'edit-buy-device');
    }
    ...
    public function fetchData($args)
    {
        $this->target = $args;
        if ($args == 'Junk') {
            $this->devices = $this->selectedCategory ? DeviceType::onlyTrashed()->where('service', ServiceType::ACCESSORIES)->get() : [];
        } else {
    ...
    {
        return view('livewire.admin.accessories.device.index')->layout('layouts.admin');
    }
}
