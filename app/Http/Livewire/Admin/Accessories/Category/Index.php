...
namespace App\Http\Livewire\Admin\Accessories\Category;
...
class Index extends Component
{
    ...
    public function updateOrderNumber($orders)
    {
        foreach ($orders as $order) {
            Category::where('id', (int)$order['value'])->update(['order_number' => $order['order']]);
        }
    ...
    public function fetchData($args)
    {
        $this->target = $args;
        if ($args == 'Junk') {
            $this->categories = Category::onlyTrashed()->where('service', ServiceType::ACCESSORIES)->get();
        } else {
            $this->categories = Category::where('service', ServiceType::ACCESSORIES)->where('status', $args)->get();
        }
    }
    ...
    public function addGradeText($categoryId)
    {
        $this->emit('categoryId', $categoryId);
        $this->emit('showM', 'add-grade-text');
    }


    public function render()
    {
        return view('livewire.admin.accessories.category.index')->layout('layouts.admin');
    }
}
