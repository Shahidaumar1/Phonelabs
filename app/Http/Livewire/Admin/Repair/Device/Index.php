...
class Index extends Component
{
    ...
    public function selectDevice(DeviceType $deivce)
    {
        ...
        $this->emit('showM', 'edit-repair-device');
    }
    ...
    public function fetchData($args)
    {
        $this->target = $args;
        if ($args == 'Junk') {
            $this->devices = $this->selectedCategory ? DeviceType::onlyTrashed()->where('service', ServiceType::REPAIR)->get() : [];
        } else {
    ...
    public function updatePriority($items)
{
    foreach ($items as $index => $item) {
        DeviceType::where('id', $item['id'])->update(['priority' => $index]);
    }

    // Refresh devices after the update
    $this->loadDevices();
}
    ...
    {
        return view('livewire.admin.repair.device.index')->layout('layouts.admin');
    }
}
